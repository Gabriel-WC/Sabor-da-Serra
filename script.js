const menuItems = [
  {
    ...
    ingredients: "Queijo curado, polvilho, ovos, leite e manteiga.", badge: "Destaque da casa", featured: true
  },
  {
  ...
  },
  {
    ...
    ingredients: "Massa artesanal, tomate, mussarela de búfala, manjericão e azeite.", badge: "🔥 Mais pedido · demo", featured: true
  },
  {
    ...
    ingredients: "Massa artesanal, calabresa, cebola caramelizada, mussarela e orégano.", badge: "Destaque da casa", featured: true
  },
  {
...
const footerMaps = $("#footerMaps");
const footerContactPlaceholder = $("#footerContactPlaceholder");
...
const productModalRatingNote = $("#productModalRatingNote");
...
function applyRestaurantConfig() {
  $$('[data-restaurant-name]').forEach((node) => { node.textContent = restaurantConfig.name; });
  $$('[data-restaurant-slogan]').forEach((node) => { node.textContent = restaurantConfig.slogan; });
  $$('[data-demo-only]').forEach((node) => { node.hidden = !restaurantConfig.isDemo; });
  $('#heroDeliveryLabel').textContent = restaurantConfig.isDemo ? 'tempo estimado • demo' : 'tempo estimado';
  $('#heroOpeningLabel').textContent = restaurantConfig.isDemo ? 'horário ilustrativo' : 'horário de funcionamento';
  $('#featuredDescription').textContent = restaurantConfig.isDemo
    ? `Uma seleção demonstrativa dos sabores que mais representam a experiência ${restaurantConfig.name}. Selos de destaque são ilustrativos nesta demo.`
    : `Uma seleção dos sabores que mais representam a experiência ${restaurantConfig.name}.`;

  const rating = Number(restaurantConfig.rating);
  const formattedRating = Number.isFinite(rating) ? rating.toFixed(1).replace('.', ',') : '—';
  $('#heroRating').textContent = formattedRating;
  $('#heroRatingLabel').textContent = restaurantConfig.reviewCount
    ? `${restaurantConfig.reviewCount} avaliações`
    : (restaurantConfig.isDemo ? 'Avaliação demonstrativa' : 'Avaliação');

  ['#heroDeliveryTime', '#trustDeliveryTime', '#footerDeliveryTime'].forEach((selector) => {
    const node = $(selector);
    if (node) node.textContent = restaurantConfig.deliveryTime || 'A confirmar';
  });
  ['#heroOpeningHours', '#trustOpeningHours', '#footerOpeningHours'].forEach((selector) => {
    const node = $(selector);
    if (node) node.textContent = restaurantConfig.openingHours || 'A confirmar';
  });
  const footerCity = $('#footerCity');
  if (footerCity) footerCity.textContent = restaurantConfig.city || 'Localização a configurar';

  const hasInstagram = Boolean(restaurantConfig.instagram);
  footerInstagram.hidden = !hasInstagram;
  if (hasInstagram) {
    const handle = restaurantConfig.instagram.replace(/^@/, '');
    footerInstagram.href = `https://instagram.com/${encodeURIComponent(handle)}`;
  }

  const whatsapp = String(restaurantConfig.whatsapp || '').replace(/\D/g, '');
  const hasWhatsapp = /^55\d{10,13}$/.test(whatsapp);
  footerWhatsapp.hidden = !hasWhatsapp;
  if (hasWhatsapp) footerWhatsapp.href = `https://wa.me/${whatsapp}`;

  const phone = String(restaurantConfig.phone || '').trim();
  const phoneDigits = phone.replace(/\D/g, '');
  footerPhone.hidden = !phoneDigits;
  if (phoneDigits) {
    footerPhone.href = `tel:+${phoneDigits}`;
    footerPhone.textContent = phone || 'Telefone';
  }

  const hasAddress = Boolean(restaurantConfig.address && restaurantConfig.city);
  footerAddress.hidden = !restaurantConfig.address;
  if (restaurantConfig.address) footerAddress.textContent = restaurantConfig.address;
  footerMaps.hidden = !hasAddress;
  if (hasAddress) {
    const destination = encodeURIComponent(`${restaurantConfig.address}, ${restaurantConfig.city}`);
    footerMaps.href = `https://www.google.com/maps/search/?api=1&query=${destination}`;
  }

  footerContactPlaceholder.hidden = hasInstagram || hasWhatsapp || Boolean(phoneDigits) || hasAddress;
  document.title = `${restaurantConfig.name} — Cardápio Artesanal`;
}
  ...
  featuredGrid.innerHTML = featured.map((item) => `
    <article class="featured-card reveal" data-id="${item.id}" tabindex="0" role="button" aria-label="Ver detalhes de ${item.name}">
      ...
      <div class="featured-card__body">
        <div class="featured-card__meta"><span>★ ${item.rating.toFixed(1).replace(".", ",")}${restaurantConfig.isDemo ? " · demo" : ""}</span><span>${item.prep}</span></div>
        <h3>${item.name}</h3>
  ...
  menuGrid.innerHTML = filtered.map((item, index) => `
    <article class="card reveal" data-id="${item.id}" tabindex="0" role="button" aria-label="Ver detalhes de ${item.name}" style="transition-delay:${Math.min(index, 6) * 35}ms">
      ...
      <div class="card__body">
        <div class="card__topline"><span>${getCategoryLabel(item.category)}</span><span class="card__rating">★ ${item.rating.toFixed(1).replace(".", ",")}${restaurantConfig.isDemo ? " · demo" : ""}</span></div>
...
function openProductModal(id, source = "menu") {
  ...
  productModalRatingNote.hidden = !restaurantConfig.isDemo;
...
function updateCartUI() {
  ...
  checkoutHint.hidden = cart.length !== 0;
...
function generateWhatsAppMessage() {
  ...
  return encodeURIComponent([
    `Olá! Gostaria de fazer um pedido no ${restaurantConfig.name}:`, "", ...lines, "",
...
function handleCheckout() {
  ...
  const whatsapp = String(restaurantConfig.whatsapp || "").replace(/\D/g, "");
  if (!/^55\d{10,13}$/.test(whatsapp)) {
    showToast("WhatsApp ainda não configurado nesta demo.");
    return;
  }
  window.open(`https://wa.me/${whatsapp}?text=${generateWhatsAppMessage()}`, "_blank", "noopener,noreferrer");
}
